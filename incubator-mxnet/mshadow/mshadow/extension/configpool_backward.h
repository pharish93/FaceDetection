...
struct Plan<ConfigPoolbExp<DataExp, GTExp, DType, srcdim>, DType>
{
	public:
		...
		MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
			...
			{
				index_t px = std::round(groundtruth_src_.Eval(gt, 0) * spatial_scale_);
				index_t py = std::round(groundtruth_src_.Eval(gt, 1) * spatial_scale_);
				for(index_t kp = 0; kp < num_keypoints_; kp++)
					for(index_t ox = 0; ox < ksize_x_; ox++)
						for(index_t oy = 0; oy < ksize_y_; oy++)
						{
							index_t x = std::round(keypoints_src_.Eval(px * src_width_ + py, kp * 2) * spatial_scale_) - ksize_x_ / 2 + ox;
							index_t y = std::round(keypoints_src_.Eval(px * src_width_ + py, kp * 2 + 1) * spatial_scale_) - ksize_y_ / 2 + oy;
