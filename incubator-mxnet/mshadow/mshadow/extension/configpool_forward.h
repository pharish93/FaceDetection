...
struct Plan<ConfigPoolfExp<DataExp, GTExp, DType, srcdim>, DType>
{
	public:
		...
		MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
			//using namespace std;
			const index_t px = std::round(groundtruth_src_.Eval(i, 0) * spatial_scale_);
			const index_t py = std::round(groundtruth_src_.Eval(i, 1) * spatial_scale_);
			...
			const index_t x = std::round(keypoints_src_.Eval(px * src_width_ + py, nk * 2) * spatial_scale_) \
							  - ksize_x_ / 2 + kx;
			const index_t y = std::round(keypoints_src_.Eval(px * src_width_ + py, nk * 2 + 1) * spatial_scale_) \
							  - ksize_y_ / 2 + ky;
