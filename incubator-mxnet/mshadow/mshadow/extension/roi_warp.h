...
struct Plan<ROIWarpExp<DataExp, GTExp, DType, srcdim>, DType>
{
	public:
		...
		MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
			...
			const index_t gt_x = std::round(gt_src_.Eval(ind, 0) * spatial_scale_);
			const index_t gt_y = std::round(gt_src_.Eval(ind, 1) * spatial_scale_);

			const index_t x = std::round(rois_src_.Eval(gt_x * src_width_ + gt_y, 0) * spatial_scale_);
			const index_t y = std::round(rois_src_.Eval(gt_x * src_width_ + gt_y, 1) * spatial_scale_);
			const index_t h = std::round(rois_src_.Eval(gt_x * src_width_ + gt_y, 2) * spatial_scale_);
			const index_t w = std::round(rois_src_.Eval(gt_x * src_width_ + gt_y, 3) * spatial_scale_);
