...
struct Plan<ROIPoolingExp<DataExp, RoisExp, DType, srcdim>, DType>
{
	public:
		...
		MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
			...
			index_t roi_start_h = std::round(rois_src_.Eval(n, r * 4) * spatial_scale_);
			index_t roi_start_w = std::round(rois_src_.Eval(n, r * 4 + 1) * spatial_scale_);
			index_t roi_end_h = std::round(rois_src_.Eval(n, r * 4 + 2) * spatial_scale_);
			index_t roi_end_w = std::round(rois_src_.Eval(n, r * 4 + 3) * spatial_scale_);

			index_t roi_height = std::max(roi_end_h - roi_start_h + 1, (index_t)1);
			index_t roi_width = std::max(roi_end_w - roi_start_w + 1, (index_t)1);
			...
			hstart = std::min(std::max(hstart + roi_start_h, (index_t)0), src_height_);
			hend = std::min(std::max(hend + roi_start_h, (index_t)0), src_height_);
			wstart = std::min(std::max(wstart + roi_start_w, (index_t)0), src_width_);
			wend = std::min(std::max(wend + roi_start_w, (index_t)0), src_width_);
			bool is_empty = (hend <= hstart) || (wend <= wstart);

			DType res = is_empty ? static_cast<DType>(0) : -FLT_MAX;
			for(index_t h = hstart; h < hend; ++h) {
				for(index_t w = wstart; w < wend; ++w) {
					res = std::max(res, data_src_.Eval(n * src_channel_ * src_height_ +
												  c * src_height_ + h, w));
				}
			}
