	...
	public:
		...
		MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
			...
			for(index_t r = 0; r < num_rois_; ++r) {
				...
				index_t roi_start_h = std::round(rois_src_.Eval(n, r * 4)
											* spatial_scale_);
				index_t roi_start_w = std::round(rois_src_.Eval(n, r * 4 + 1)
											* spatial_scale_);
				index_t roi_end_h = std::round(rois_src_.Eval(n, r * 4 + 2)
										  * spatial_scale_);
				index_t roi_end_w = std::round(rois_src_.Eval(n, r * 4 + 3)
										  * spatial_scale_);
				...
				phstart = std::min(std::max(phstart, (index_t)0), pooled_shape_[0]);
				phend = std::min(std::max(phend, (index_t)0), pooled_shape_[0]);
				pwstart = std::min(std::max(pwstart, (index_t)0), pooled_shape_[1]);
				pwend = std::min(std::max(pwend, (index_t)0), pooled_shape_[1]);
