...
class GenEllLabelOp : public Operator {
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        CHECK_EQ(in_data.size(), 3) << "GenEllLabelOp Input: [rois, label, groundtruth]";
...
class GenEllLabelProp : public OperatorProperty {
	public:
		...
		bool InferShape(std::vector<TShape> *in_shape,
			...
			CHECK_EQ(in_shape->size(), 3) << "Input: [rois, label, groundtruth]";
			...
			CHECK_EQ(gtshape.ndim(), 2) << \
				"GenEllLabel: Input groundtruth should be 2D in (num_gt, 2)";
